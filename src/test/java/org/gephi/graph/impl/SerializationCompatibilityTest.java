 ...
 * <ol>
 * <li><b>Backward compatibility</b> - every fixture, from the oldest minor to the current one, must deserialize and
 * yield the expected content.</li>
 * <li><b>Format drift</b> - for the <i>current</i> minor only, serializing the model built by
 * {@link SerializationFixtureGenerator} must produce bytes identical to the committed file. Older minors are
 * deliberately exempt: we no longer write those formats.</li>
 * <li><b>Determinism</b> - serializing the same content twice must produce identical bytes, regardless of the order the
 * content was built in.</li>
 * </ol>
 *
 * See <code>src/test/resources/serialization/README.md</code> before touching any fixture file.
 */
public class SerializationCompatibilityTest {
    ...
    //
    // The pin compares what the write path produces today against the committed golden file. It deliberately does not
    // take the "read the fixture back and re-serialize it" shape: deserialization is not byte-idempotent in this
    // codebase (GraphVersion counters and TimeIndexStore.countMap are both restored from the stream and then bumped
    // again as elements are re-inserted, and TextProperties width/height are dropped on read), so no fixture could
    // ever be a fixed point. The read path is covered by the content assertions in contract 1 instead.
    ...
    public void testCurrentMinorIsByteIdentical(String fixture) throws IOException {
        ...
        assertBytesEqual(committed, regenerated, "Serialization format drift detected for fixture " + path(CURRENT_MINOR, fixture) + ".\nSerializing the model built by SerializationFixtureGenerator no longer reproduces the" + " bytes of the committed file.\nEither this is a bug, or the format changed on purpose - in which" + " case the version must be bumped\nand the fixtures regenerated deliberately. Do not regenerate just" + " to make this test green;\nsee src/test/resources/serialization/README.md.");
    }
    ...
    public void testGraphAttributesOrderIsCanonical() throws IOException {
        // The same graph attributes inserted in opposite orders must serialize identically. This is what makes the
        // byte-pinned fixtures stable, see GraphAttributesImpl.
    ...
    // Round-trip only, no byte assertions: the hash-ordered surface deliberately kept out of the fixtures
    ...
    private void assertStaticValues(org.gephi.graph.api.Element element) {
        ...
        // element values survive but the array component type does not. Pinned here on purpose.
